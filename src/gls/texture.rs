...
#[rustfmt::skip]
#[inline(never)]
pub fn load_dds_to_opengl(path: &Path) -> GLuint {
    let (image_data, format, mut width, mut height, mipmap_count) = load_dds(path);
...
#[rustfmt::skip]
#[inline(never)]
pub fn load_cubemap(path: &[&Path; 6]) -> GLuint {
    unsafe {
        let mut cubemap: GLuint = 0;
		...
		gl::ActiveTexture(gl::TEXTURE0 + cubemap);
