...
#[inline(never)]
pub fn read_skn(contents: &Vec<u8>) -> Skin {
    ...
    if major > 0 {
        ...
        for _ in 0..submeshheader_count {
            ...
            let vertex_offset = reader
                .read_u32::<LittleEndian>()
                .expect("Could not read SKN SubMeshHeader vertex offset");
            let vertex_count = reader
                .read_u32::<LittleEndian>()
                .expect("Could not read SKN SubMeshHeader vertex count");
    ...
    if submeshheader_count > 0 {
        ...
        for submeshheader in submeshheaders {
            let hash = lol::hasher::fnv1a(&submeshheader.name);
